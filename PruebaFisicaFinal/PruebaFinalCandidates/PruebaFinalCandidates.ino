...
void loop() {
  ...
  // if(detectarColor() == 7) {
  antiwallContinuousMovement2(motorIzq, motorDer, 150);
...
void antiwallMovement(Motor& motor1, Motor& motor2, int v) { // -------------- Funcion Pensada para Laberinto de COLORES
  ...
  while (true) {
    ...
    //   motorsOFF(true); // Apaga los motores
    //   avanzarMotoresSincronizados(motor1, motor2, 1, 1, true, true); // Giro de 180°
    // }
    // // if(count == 0) {
    ...
    if (((motor1.calcularRevoluciones() >= setpoint1 || motor2.calcularRevoluciones() >= setpoint2) && (vueltasDer == 1 && vueltasIzq == 1)) && distFront > minDist && paso) {
      ...
      if (getDistance(trigPin3, echoPin3) >= maxSide + 20 || getDistance(trigPin1, echoPin1) >= maxSide + 20) {
        if (lastVuelta == 1) {
          ...
          motor1.cambiarVelocidad(v * 0.3); // Ajusta la velocidad del motor 1
          motor2.cambiarVelocidad(v * 0.3); // Ajusta la velocidad del motor 2
          estados(2); // Cambia el estado a 1
          avanzarMotoresSincronizados(motor1, motor2, 1.4, 1.4, true, true); // Giro de 90° a la derecha
          avanzarMotoresSincronizados(motor1, motor2, 0.5, 0.5, true, false); // Avanza en línea recta
          if (distFront > minDist) {
            estados(1); // Cambia el estado a 1
...
void antiwallContinuousMovement2(Motor& motor1, Motor& motor2, int v) { // ---------------- Funcion FINAL para Laberinto con pelota
  ...
  while (true) {
    ...
    if ((distFront > minDist + 20) && pasoFront && ((motor1.calcularRevoluciones() >= 3 || motor2.calcularRevoluciones() >= 3) && (vueltasDer == 0 && vueltasIzq == 0)) && pelota == false) {
      estados(3); // Cambia el estado a 3
      motorsOFF(true); // Apaga los motores
      estados(3); // Cambia el estado a 3
      motorsOFF(false); // Apaga los motores
      ...
      motor1.cambiarVelocidad(70); // Ajusta la velocidad del motor 1
      motor2.cambiarVelocidad(70); // Ajusta la velocidad del motor 2
      avanzarMotoresSincronizados(motor1, motor2, 3.5, 3.5, true, true); // Giro de 180° a la derecha
      delay(1000); // Espera 1 segundo
      avanzarMotoresSincronizados(motor1, motor2, 2.7, 2.7, false, true); // Avanza en línea recta
      delay(1000);
      servo(true); // Mueve el servo a la posición 1000
      ...
      motor1.cambiarVelocidad(70); // Ajusta la velocidad del motor 1
      motor2.cambiarVelocidad(70); // Ajusta la velocidad del motor 2
      avanzarMotoresSincronizados(motor1, motor2, 2, 2, true, false); // Avanza en línea recta
      delay(1000);
      avanzarMotoresSincronizados(motor1, motor2, 1.4, 1.4, true, true); // Avanza en línea recta
      pasoFront = false; // Cambia la bandera a false
    }
    if (((motor1.calcularRevoluciones() >= setpoint1 || motor2.calcularRevoluciones() >= setpoint2) && (vueltasDer == 1 && vueltasIzq == 1)) && distFront > minDist && paso && pelota == false) {
      estados(3); // Cambia el estado a 3
      motorsOFF(true); // Apaga los motores
      estados(3); // Cambia el estado a 3
      motorsOFF(false); // Apaga los motores
    ...
    if (((motor1.calcularRevoluciones() >= setpoint1 || motor2.calcularRevoluciones() >= setpoint2) && (vueltasDer == 2 || vueltasIzq == 2)) && distFront > minDist && paso && pelota == false) {
      estados(3); // Cambia el estado a 3
      motorsOFF(true); // Apaga los motores
      estados(3); // Cambia el estado a 3
      motorsOFF(false); // Apaga los motores
    ...
    if (((motor1.calcularRevoluciones() >= setpoint1 || motor2.calcularRevoluciones() >= setpoint2) && (vueltasDer >= 3 || vueltasIzq >= 3)) && distFront > minDist && paso && pelota == false) {
      estados(3); // Cambia el estado a 3
      motorsOFF(true); // Apaga los motores
      estados(3); // Cambia el estado a 3
      motorsOFF(false); // Apaga los motores
    ...
    if (pelota == false && (vueltasDer >= 3 || vueltasIzq >= 3)) {
      vueltasDer = 0;
      vueltasIzq = 0;
    }
...
void antiwallContinuousMovement3(Motor& motor1, Motor& motor2, int v) { // ---------------- Funcion FINAL para Laberinto con pelota
  const int minDist = 9; // Distancia mínima en cm para corrección
  const int minSide = 6; // Distancia mínima lateral en cm para corrección
  const int maxSide = 9; // Distancia máxima lateral en cm para corrección
  ...
  while (true) {
    ...
    } else if(getDistance(trigPin2, echoPin2) <= minDist) {
      // Si hay un obstáculo al frente, decidir la dirección en función del espacio a los lados
      motor1.cambiarVelocidad(v * 0.3); // Ajusta la velocidad del motor 1
      motor2.cambiarVelocidad(v * 0.3); // Ajusta la velocidad del motor 2
      if (distLeft > distRight || !pasoFront) {
        // Hay más espacio a la izquierda: girar a la izquierda
        Serial.println("Girar a la Izquierda");
        avanzarMotoresSincronizados(motor1, motor2, 1.4, 1.4, false, false); // Giro de 90° a la izquierda
        delay(500); // Espera 1 segundo
    ...
    if ((distFront > minDist + 20) && pasoFront && ((motor1.calcularRevoluciones() >= 3 || motor2.calcularRevoluciones() >= 3) && (vueltasDer == 0 && vueltasIzq == 0))) {
      estados(3); // Cambia el estado a 3
      motorsOFF(true); // Apaga los motores
      estados(3); // Cambia el estado a 3
      motorsOFF(false); // Apaga los motores
